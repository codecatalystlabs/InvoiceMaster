<?php
/**
 * Code Catalyst Labs - Create Quotation
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Insert quotation
    $query = "INSERT INTO quotations (client_id, quotation_number, date, subtotal, tax, discount, total, status, notes) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "issddddss", $client_id, $quotation_number, $date, $subtotal, $tax, $discount, $total, $status, $notes);
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        foreach ($items as $item) {
            if (!empty($item['item_name']) && $item['qty'] > 0 && $item['unit_price'] > 0) {
                ...
                mysqli_stmt_bind_param($item_stmt, "isidd", $quotation_id, $item['item_name'], $item['qty'], $item['unit_price'], $item_total);
        ...
        logAudit($conn, 'Create', 'Quotation', $quotation_id, "Quotation $quotation_number created");
...
<div class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="bi bi-file-earmark-text"></i> Create Quotation</h2>
